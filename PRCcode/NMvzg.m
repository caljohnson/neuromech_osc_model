...
% %second-difference matrix A
% A = 1/dim.*spdiags([ones(dim,1) -2*ones(dim,1) ones(dim,1)], [0 1 2], dim, dim+2);
% B = inv(full(A*A'));
% 
% %check if antiphase is a solution to the mech coupled only case
% gm_phi = zeros(dim,1);
% for k = 1:dim
%     gm_phi(k) = 0;
%     for jj=1:dim
%         if mod(jj+k,2)==0
%             gm_phi(k) = gm_phi(k) + gamma*B(k,jj)*h1m(1);
%         else
%             gm_phi(k) = gm_phi(k) + gamma*B(k,jj)*h1m(end/2);
%         end
%     end
% end
% 
% % calculate G-functions for the chain
% % ---------------------
% h1ms = repmat(h1m(1,:),[dim,1]);
% gm_phi = zeros(dim-1,size(h1m,2));
% gp_phi = zeros(dim-1,size(h1m,2));
% gp_phi(1,:) = h1p(1,:);
% for jj=1:dim-1
%     gm_phi(jj,:) = -B(jj,:)*h1ms + B(jj+1,:)*h1ms;
% end
% 
% figure(5); clf;
% for jj=1:dim-1
%     plot(t(1:ii), gm_phi(jj,:),'linewidth',2); hold on; 
% end
% hold off;
% return
% % plot G-functions for different rho = gamma/eps
...
% % "animated"
% for rho=0:0.1:2
% g=eps_prop*(2*gp_phi + rho*gm_phi);
% [~, Inds] = sort(abs(g));
% figure(4)
% clf;
% plot(t(1:ii),g(1,:),'linewidth',2);
% hold on;
% plot([0,p],[0,0],'k:','linewidth',2);
% plot(t(1:ii),eps_prop*gm_phi(1,:),'r-','linewidth',2);
% plot(t(1:ii),2*eps_prop*gp_phi(1,:),'g-','linewidth',2);
% % plot(t(Inds(1:3)),0*Inds(1:3),'ko','Markersize',10);
% % plot(t(Indsg1(1:3)),0*Indsg1(1:3),'ko','Markersize',10);
% % plot(t(Indsh1p(1:2)),0*Indsh1p(1:2),'ko','Markersize',10);
% legend('mechanical+proprioceptive','"zero"','mechanical','proprioceptive');
% xlabel('time(ms)'); 
% str1=['G-function for NM paired-oscillator chain; \gamma /\epsilon= ' num2str(rho)];
% title(str1);
% pause(1.0);
% end;